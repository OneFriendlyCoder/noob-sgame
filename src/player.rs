...
impl Player {
    ...
    pub fn draw_player(&self, camera_view: &CameraView) {
        if let CameraView::ThirdPerson = camera_view {
            draw_cube(self.position, self.size, None, RED);
        }
    }
    ...
    pub fn update_player_position(
        ...
        strafe_dir: Vec3,
        look: Vec3,
        enemies: &mut Vec<Player>, // ✅ changed from Enemies
        grid: &mut Grid,
        camera: &mut Camera3D,
        camera1: &mut Camera3D,
        ...
        // ✅ detect collision with Vec<Player>
        if detect_collision(enemies, grid, self) {
            println!("Collision detected");
            self.position = previous_position;
        }
        ...
        let target_fovy = if is_mouse_button_down(MouseButton::Right) {
            ...
            if is_mouse_button_down(MouseButton::Left) {
                ...
                if let Some(hit_idx) = check_bullet_hit_grid(enemies, grid, self) {
                    println!("Bullet hit enemy {}", hit_idx);
                }
            }
            45.0_f32.to_radians()
        } else {
